#!/usr/bin/perl -w
#######################################################################
#Iterative QA (copied from consensus_qa.robust.pl)
...
#enlarge the pool because our model pool is small than casp model pool
$num = int($num * 1.6); 
...
#		<STDIN>;	
	...
	if (@ca_x != $slen)
	{
		 warn "$model: sequence length does not match with the number of CA atoms.\n";
		next;
	}
...
for ($i = 1; $i <= 1; $i++)
{
	if (-f "closc$i.pdb")
	{
		...
		foreach $model (@selected_models)
		{
			...
			$out = `/home/jh7x3/multicom_v1.1/tools/tm_score/TMscore_32 $model_file closc$i.pdb`;
			@out = split(/\n+/, $out);
			$found = 0; 
			foreach $line (@out)
			{
...
for ($i = 0; $i < @model_files; $i++)
{
	...
	$reference_model_num = 10;
	#for ($j = 0; $j < 5 && $j < @new_rank; $j++)
	for ($j = 0; $j < $reference_model_num && $j < @new_rank; $j++)
	{	
		$record = $new_rank[$j];
		@fields = split(/\s+/, $record);
...
#	print OUT " ";
#	print OUT $sorted_model_scores[$i]{"rmsd"};
#	print "\n";
	print OUT "\n";
}  
print OUT "END\n";
