...
if (@ARGV != 5)
{
	die "need 5 parameters: prosys_dir (/home/jh7x3/multicom_v1.1/src/prosys/), modeller_dir (/home/jh7x3/multicom_v1.1/tools/modeller9v7/),  fasta file, multicom output directory (~/casp_roll/T0663), and number of domains\n";
}
...
foreach $model (@models)
{
	for ($jj = 0; $jj < 5; $jj++)
	{
		...
		if ($model eq "construct" && $jj == 0)
		{
			...
			`cp $comb_dir/casp1-$model$jj.pdb $output_dir/compo/caspb1.pdb`; 
			`cp $comb_dir/clash-$model$jj.txt $output_dir/compo/caspb1.txt`; 
		}	
	}
