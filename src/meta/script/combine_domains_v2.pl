...
foreach $model (@models)
{
	...
	if ($model eq "cluster")
	{
		...
		`cp $comb_dir/casp1.pdb $output_dir/cluster/casp1.pdb`; 
		`cp $comb_dir/clash1.txt $output_dir/cluster/clash1.txt`; 
	}	
	...
	system("/home/jh7x3/multicom_v1.1/src/meta/model_cluster/script/clash_check.pl $fasta_file $model_file.scw > $comb_dir/clash-$model$jj.txt");

	#$pdb2casp2 = "/home/casp13/MULTICOM_package/casp8/meta/script/pdb2casp.pl";
	$pdb2casp2 = "/home/jh7x3/multicom_v1.1/src/meta/script/pdb2casp.pl";
	...
	if ($model eq "construct" && $jj == 0)
	{
		`mv $output_dir/cluster/con_casp1.pdb $output_dir/cluster/con_casp1.pdb.org`; 	
		`cp $comb_dir/casp1-$model$jj.pdb $output_dir/cluster/con_casp1.pdb`; 
		`cp $comb_dir/clash-$model$jj.txt $output_dir/cluster/con_clash1.txt`; 
	}	
	...
	if ($model eq "novel" && $jj == 0)
	{
		`mv $output_dir/select/casp1.pdb $output_dir/select/casp1.pdb.org`; 	
		`cp $comb_dir/casp1-$model$jj.pdb $output_dir/select/casp1.pdb`; 
		`cp $comb_dir/clash-$model$jj.txt $output_dir/select/clash1.txt`; 
	}	
