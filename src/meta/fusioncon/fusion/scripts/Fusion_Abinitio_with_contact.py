...
############################################################################
#
#	 Method to perform abinitio modeling (TS)
#
############################################################################
import sys
project_root = '/home/jh7x3/multicom_v1.1/tools/Fusion/'
...
print '  ############################################################################'
print '  #                                                                          #'
print '  #   Method to perform abinitio modeling (TS)                               #'
print '  #                                                                          #'
print '  ############################################################################'
print ''
...
os.chdir(work_dir)
modeling = FusionModeling_Lite()
modeling.set_target(target)
modeling.set_reply_email(email)
modeling.set_simulation_timeout(timeout)
if constraint:
	print "Loading custom constraints ",constraint
	modeling.set_custom_constraint(constraint)
modeling.set_cpu(cpu)
modeling.set_decoy_per_cpu(decoy_per_cpu)
modeling.set_num_model(model)
modeling.set_cycle_factor(increase_cycle)
if native:
	modeling.set_native(native_pose)
modeling.set_author(author)
modeling.apply(pose)
