...
$MAX_ALIGN = 100; 
$highest_num = 300; #support at most 300 local alignments to control the maximum time
...
$align_dir="/home/jh7x3/multicom_v1.1/src/meta/script"; 
...
foreach $local (@local_list)
{
	...
	foreach $select (@new_local_list)
	{
		...
		elsif ($local->{"qstart"} >= $select->{"qstart"} && $local->{"qend"} <= $select->{"qend"})
		{
			$repeat++; 
		}
	}	
...
$total_num = @new_local_list;
print "total number of selected local alignments is $total_num\n";

#set an index for each entry
for ($i = 0; $i < @new_local_list; $i++)
{
	$new_local_list[$i]->{"idx"} = $i; 
	print "$i: ",  $new_local_list[$i]->{"tname"}, "\n"; 
}
...
sub compare_locals
{
	...
	if (! -f $pdb1)
	{
		`cp $atom_dir/$tname1.atom.gz .`; 
		`gunzip $tname1.atom.gz`; 
		`mv $tname1.atom $tname1.atm`; 
	}
	
	if (! -f $pdb2)
	{
		`cp $atom_dir/$tname2.atom.gz .`; 
		`gunzip $tname2.atom.gz`; 
		`mv $tname2.atom $tname2.atm`; 
	}
...
use lib "/home/jh7x3/multicom_v1.1/src/meta/script/";
use Align; 
...
for ($i = 0; $i < @new_local_list; $i++)
{
	...
	if ($left_ext_len > 0 &&  $local->{"ltail_len"} <= $max_extension_length)
	{
		#do left extension
	...
	if ($right_ext_len > 0 && $local->{"rtail_len"} <= $max_extension_length)
	{
		#do left extension
