#!/usr/bin/perl -w
#perl /home/jh7x3/multicom/installation/scripts/validate_integrated_predictions.pl  T0993s2  /home/jh7x3/multicom/test_out/T0993s2_out /home/jh7x3/multicom//installation/benchmark
if (@ARGV != 3) {
...
opendir(FILES,"$test_dir/full_length/meta/") || die "Failed to open directory $test_dir/full_length/meta/\n";
@files = readdir(FILES);
closedir(FILES);
$avg_gdt_predict = 0;
...
foreach $file (@files)
{
	...
	$predict_file = "$test_dir/full_length/meta/$file";
	$benchmark_file = "$benchmark_dir/$targetid/$file";
	...
	($tmscore1,$gdttsscore1,$rmsd1) = cal_sim($predict_file,$native_file);
	$avg_gdt_predict += $gdttsscore1;
	
	($tmscore2,$gdttsscore2,$rmsd1) = cal_sim($benchmark_file,$native_file);
